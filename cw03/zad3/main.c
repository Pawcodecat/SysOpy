...
int main(int argc, char** argv)
{
    ...
    int virtualMemLimit = atoi(argv[6]);
    ...
    for(int i = 0; i < procNumber; i++)
    {
        pid_t childPid = fork();
        if(childPid == 0) {
            struct rlimit* mLimit = (struct rlimit*)malloc(sizeof(struct rlimit));
            mLimit->rlim_cur = virtualMemLimit * (1 << 20);
            mLimit->rlim_max = virtualMemLimit * (1 << 20);
            setrlimit(RLIMIT_AS, mLimit);
            struct rlimit* tLimit = (struct rlimit*)malloc(sizeof(struct rlimit));
            tLimit->rlim_cur = cpuTimeLimit;
            tLimit->rlim_max = cpuTimeLimit;

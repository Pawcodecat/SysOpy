...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
        ...
        recvfrom(socket_fd, buffer, MAX_MESSAGE_LENGTH, 0, &from_addr, &from_len);
        ...
        if(strcmp(cmd, "add") == 0)
        {
            ...
            else
            {
                ...
                sendto(clients[first]->fd, "add:0", MAX_MESSAGE_LENGTH, 0,
        ...
        if(strcmp(cmd, "move") == 0)
        {
            ...
            sendto(clients[get_rival(player)]->fd, buffer, MAX_MESSAGE_LENGTH, 0,
            &clients[get_rival(player)]->addr, sizeof(struct addrinfo));
        }
